...
import { peakCallLoadStrategy, PeakCallLoadMetadata } from '@/lib/autoExclusions/strategies/peakCallLoad';
...
interface DetectRequest {
  ...
  endDate?: string;
  callId?: number;
  applyExclusions?: boolean; // If true, actually apply exclusions to DB
}
...
export async function POST(req: NextRequest) {
  ...
  // Validate input
  if (!callId && !parishId) {
    return NextResponse.json({ error: 'Either callId or parishId is required' }, { status: 400 });
  }

  const client = await pool.connect();
  try {
    // Get calls to evaluate
    let callsToEvaluate: Array<{
      id: number;
    ...
    if (callId) {
      ...
      const result = await client.query(`
        SELECT id, response_number, parish_id, region_id, response_area,
               response_date_time, compliance_time_minutes, threshold_minutes
        FROM calls 
        WHERE parish_id = $1
          AND compliance_time_minutes > (threshold_minutes + 0.9833) -- Out of compliance (X:59 rule)
          AND is_excluded = FALSE
          AND is_auto_excluded = FALSE
          ${dateFilter}
        ORDER BY call_in_que_time ASC
      `, params);
      callsToEvaluate = result.rows;
    }
    ...
    for (const call of callsToEvaluate) {
      ...
      } else if (metadata.decision === 'HUMAN_REVIEW') {
        flaggedForReview.push({ call, result });
      }
    }
    ...
    if (applyExclusions && excluded.length > 0) {
      ...
      try {
        for (const item of excluded) {
          ...
          // Update the call record - set BOTH is_excluded and is_auto_excluded
          await client.query(`
            UPDATE calls SET
              is_excluded = TRUE,
          ...
          await client.query(`
            INSERT INTO audit_logs (
              ...
              'AUTO_EXCLUSION', 'call', $1::text, $2, $3
            )
          `, [call.id, `Auto-excluded call ${call.response_number} via PEAK_CALL_LOAD`, JSON.stringify({
            strategyKey: 'PEAK_CALL_LOAD',
            parishName: metadata.parishName,
            callPosition: metadata.callPosition,
            callsInWindow: metadata.callsInWindow,
          })]);
        }

        // Also flag human review calls
        for (const item of flaggedForReview) {
          ...
          await client.query(`
            UPDATE calls SET
              ...
              human_review_reason = $2,
              human_review_flagged_at = NOW()
            WHERE id = $1
          `, [call.id, result.reason]);
        }
    ...
    return NextResponse.json({
      ...
      excluded: excluded.map(e => ({
        ...
        decision: (e.result.metadata as PeakCallLoadMetadata).decision,
      })),
      flaggedForReview: flaggedForReview.map(e => ({
        callId: e.call.id,
        responseNumber: e.call.response_number,
        reason: e.result.reason,
        decision: (e.result.metadata as PeakCallLoadMetadata).decision,
    ...
    console.error('Error detecting peak call load:', err);
    return NextResponse.json(
      { error: 'Failed to detect peak call load', details: err.message },
      { status: 500 }
    );
  } finally {
    client.release();
