...
const REGIONS = [
  { id: 'cenla', name: 'Central Louisiana (CENLA)', areaType: 'Parish', areaTypePlural: 'Parishes' },
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <div>
              ...
              <p className="text-sm text-slate-500">Uploading to: <span className="font-medium text-[#004437]">{regionName}</span></p>
            </div>
            <button
              onClick={handleClose}
              disabled={uploading}
          ...
          <div className="px-6 py-5">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-5">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-amber-800">
                  <p className="font-medium mb-1">Export Calls from MicroStrategy for the ENTIRE region, set export to CSV ONLY, any other file format will not work.</p>
                </div>
              </div>
            </div>
            ...
            {testResult && (
              <div className="mt-4 space-y-3">
                ...
                {/* Parish Breakdown */}
                {testResult.parishBreakdown && Object.keys(testResult.parishBreakdown).length > 0 && (
                ...
                {testResult.unknownResponseAreas && Object.keys(testResult.unknownResponseAreas).length > 0 && (
                  <div className="p-3 bg-slate-100 rounded-lg">
                    <h4 className="font-medium text-slate-700 mb-2">Response Areas → "Other" tile</h4>
...
function Dashboard({ user, onLogout }) {
    ...
    const fetchRegions = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          const accessible = userHasAllRegions || userIsAdmin
            ? data
  ...
  const accessibleRegions = userHasAllRegions || userIsAdmin
    ? regions
  ...
  // Date range state - defaults to previous month
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        if (data.ok && data.stats) {
          ...
          const totalCalls = Object.values(data.stats).reduce((sum, p) => sum + (p.totalCalls || 0), 0);
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                {regionsLoading || !selectedRegion ? (
                  ...
                  >
                    {selectedRegion.name}
                    <svg className={`w-5 h-5 transition-transform ${regionMenuOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </button>
                ) : (
                    ...
                    {accessibleRegions.map((region) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-100 flex items-center justify-between ${
                          selectedRegion.id === region.id ? 'text-[#004437] font-medium bg-emerald-50' : 'text-slate-700'
                        }`}
                      >
                        {region.name}
            ...
            <div className="text-sm text-slate-500 bg-slate-100 px-4 py-2 rounded-lg">
              ...
              <span className="text-slate-600 font-medium">{lastUpdated.date} {lastUpdated.time}</span>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-center">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-sm text-slate-500 mb-6">
                  There is no compliance data for <span className="font-medium">{selectedRegion?.name || 'this region'}</span> in <span className="font-medium">{displayMonthName}</span>.
                </p>
                <button
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2">
            {parishes.map((key) => {
              ...
              const areaTypePlural = areaType === 'Parish' ? 'Parishes' : areaType === 'County' ? 'Counties' : `${areaType}s`;
              const hasData = parish?.totalCalls > 0;

              // Find the parish info from the region's parishes list (for logo_url)
              const regionParish = selectedRegion?.parishes?.find(p =>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            ...
            <button
              type="button"
              onClick={() => {
                const regionCode = selectedRegion?.name?.toUpperCase().replace(/\s+/g, '') || 'CENLA';
                const shortCode = regionCode === 'CENTRALLOUISIANA' ? 'CENLA' :
                                  regionCode === 'SOUTHWESTLA' ? 'SWLA' : regionCode;
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 truncate">
                  Call density & optimized posting
                </p>
              </div>

              {/* Arrow */}
              <svg className="w-5 h-5 text-slate-400 group-hover:text-[#004437] group-hover:translate-x-1 transition-transform flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              ...
              {/* Arrow */}
              <svg className="w-5 h-5 text-slate-400 group-hover:text-slate-600 group-hover:translate-x-1 transition-transform flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
            </button>
          </div>
...
export default function AcadianDashboardPage() {
  ...
  // Handler for logout - uses NextAuth signOut
  const handleLogout = () => {
  ...
  const user = {
    displayName: session.user?.display_name || session.user?.name || session.user?.email?.split('@')[0] || 'User',
